...
#define TIME_FROM_SEC(sec)                                                     \
	{                                                                      \
		{                                                              \
			.tv_nsec = 0, .tv_sec = sec                            \
		}                                                              \
	}
...
static bool opt_deprecated_ok(struct lightningd *ld, const char *argname,
			      const char *issue, const char *start,
			      const char *end)
{
	return lightningd_deprecated_in_ok(ld, ld->log, ld->deprecated_ok,
					   argname, issue, start, end, NULL);
}
...
char *opt_set_autobool_arg(const char *arg, enum opt_autobool *b)
{
	...
	if (!strcasecmp(arg, "auto") || !strcasecmp(arg, "default")) {
		*b = OPT_AUTOBOOL_AUTO;
		return NULL;
	}
					    ...
					    struct lightningd *ld)
{
	...
	if (!opt_deprecated_ok(ld, "accept-htlc-tlv-types", NULL, "v23.08",
			       "v24.08")) {
...
static size_t num_announced_types(enum wire_addr_type type,
				  struct lightningd *ld)
{
	size_t num = 0;
...
static char *opt_add_addr_withtype(const char *arg, struct lightningd *ld,
				   enum addr_listen_announce ala)
{
	...
	err_msg = parse_wireaddr_internal(tmpctx, arg, ld->portnum,
					  dns_lookup_ok, &wi);
	if (err_msg)
		return tal_fmt(tmpctx, "Unable to parse address '%s': %s", arg,
			       err_msg);
	...
	case ADDR_INTERNAL_WIREADDR:
		...
		case ADDR_TYPE_IPV6:
			if ((ala & ADDR_ANNOUNCE) && wi.u.allproto.is_websocket)
				return tal_fmt(tmpctx,
					       "Cannot announce websocket "
					       "address, use --bind-addr=%s",
					       arg);
		...
		case ADDR_TYPE_TOR_V3:
			...
			case ADDR_LISTEN:
				if (!opt_deprecated_ok(ld, "bind-addr", "torv3",
					...
					return tal_fmt(
					    tmpctx,
					    "Don't use --bind-addr=%s, use "
					    "--announce-addr=%s",
					    arg, arg);
				log_unusual(
				    ld->log,
				    "You used `--bind-addr=%s` option with an "
				    ".onion address,"
				    " You are lucky in this node live some "
				    "wizards and"
				    " fairies, we have done this for you and "
				    "don't announce, Be as hidden as wished",
				    arg);
				/* And we ignore it */
				return NULL;
			case ADDR_LISTEN_AND_ANNOUNCE:
				...
				log_unusual(
				    ld->log,
				    "You used `--addr=%s` option with an "
				    ".onion address,"
				    " You are lucky in this node live some "
				    "wizards and"
				    " fairies, we have done this for you and "
		...
		case ADDR_TYPE_DNS:
			...
			case ADDR_LISTEN:
				return tal_fmt(
				    tmpctx,
				    "Cannot use dns: prefix with --bind-addr, "
				    "use --bind-addr=%s",
				    arg + strlen("dns:"));
			case ADDR_LISTEN_AND_ANNOUNCE:
				return tal_fmt(
				    tmpctx,
				    "Cannot use dns: prefix with --addr, use "
				    "--bind-addr=%s and --addr=%s",
				    arg + strlen("dns:"), arg);
			}
			/* BOLT #7:
			 * The origin node:
			 * ...
			 *   - MUST NOT announce more than one `type 5` DNS
			 * hostname.
			 */
			if (num_announced_types(ADDR_TYPE_DNS, ld) > 0)
				return tal_fmt(tmpctx,
					       "Only one DNS can be announced");
			break;
		}
		break;
	case ADDR_INTERNAL_SOCKNAME:
		...
		case ADDR_LISTEN_AND_ANNOUNCE:
			if (!opt_deprecated_ok(ld, "addr", "socket", "v23.08",
					       "v24.08"))
				return tal_fmt(
				    tmpctx,
				    "Don't use --addr=%s, use --bind-addr=%s",
				    arg, arg);
			ala = ADDR_LISTEN;
			/* Fall thru */
		case ADDR_LISTEN:
	...
	case ADDR_INTERNAL_ALLPROTO:
		...
		case ADDR_ANNOUNCE:
			return tal_fmt(tmpctx,
				       "Cannot use wildcard address '%s'", arg);
		case ADDR_LISTEN_AND_ANNOUNCE:
	...
	case ADDR_INTERNAL_FORPROXY:
		...
		return tal_fmt(tmpctx,
			       "Cannot resolve address '%s' (not using DNS!)",
			       arg);
	}
...
static char *opt_add_plugin(const char *arg, struct lightningd *ld)
{
	...
	if (!p)
		return tal_fmt(tmpctx, "Failed to register %s: %s", arg,
			       strerror(errno));
	return NULL;
}
...
static char *opt_set_hsm_password(struct lightningd *ld)
{
	...
	prompt(ld,
	       "The hsm_secret is encrypted with a password. In order to "
	...
	if (!is_encrypted) {
		...
		fflush(stdout);
		passwd_confirmation =
		    read_stdin_pass_with_exit_code(&err_msg, &opt_exitcode);
	...
	opt_exitcode = hsm_secret_encryption_key_with_exitcode(
	    passwd, ld->config.keypass, &err_msg);
	if (opt_exitcode > 0)
		return cast_const(char *, err_msg);
...
{
	if (!opt_deprecated_ok(ld, "max-locktime-blocks", NULL, "v24.05",
			       "v24.11"))
		return "--max-locktime-blocks has been deprecated (BOLT #4 "
		       "says 2016)";
...
static char *opt_force_privkey(const char *optarg, struct lightningd *ld)
{
	...
	if (!hex_decode(optarg, strlen(optarg), ld->dev_force_privkey,
			sizeof(*ld->dev_force_privkey)))
...
static char *opt_force_bip32_seed(const char *optarg, struct lightningd *ld)
{
	...
	if (!hex_decode(optarg, strlen(optarg), ld->dev_force_bip32_seed,
...
static char *opt_force_tmp_channel_id(const char *optarg, struct lightningd *ld)
{
	...
	if (!hex_decode(optarg, strlen(optarg), ld->dev_force_tmp_channel_id,
			sizeof(*ld->dev_force_tmp_channel_id)))
		return tal_fmt(tmpctx, "Unable to parse channel id '%s'",
			       optarg);
	return NULL;
}
...
static char *opt_force_featureset(const char *optarg, struct lightningd *ld)
{
	...
	if (tal_count(parts) != NUM_FEATURE_PLACE + 1) {
		if (!strstarts(optarg, "-") && !strstarts(optarg, "+"))
			return "Expected 8 feature sets (init/globalinit/"
			       " node_announce/channel/bolt11/b12offer/"
			       "b12invreq/b12inv) each terminated by /"
			       " OR +/-<single_bit_num>";
...
static char *opt_ignore(void *unused) { return NULL; }
...
static void dev_register_opts(struct lightningd *ld)
{
	...
	clnopt_witharg("--dev-debugger=<subprocess>", OPT_EARLY | OPT_DEV,
		       opt_set_charp, opt_show_charp, &ld->dev_debug_subprocess,
		       "Invoke gdb at start of <subprocess>");
	clnopt_noarg("--dev-no-plugin-checksum", OPT_EARLY | OPT_DEV,
		     opt_set_bool, &ld->dev_no_plugin_checksum,
		     "Don't checksum plugins to detect changes");
	clnopt_noarg(
	    "--dev-builtin-plugins-unimportant", OPT_EARLY | OPT_DEV,
	    opt_set_bool, &ld->plugins->dev_builtin_plugins_unimportant,
	    "Make builtin plugins unimportant so you can plugin stop them.");
	clnopt_noarg("--dev-no-reconnect", OPT_DEV, opt_set_invbool,
		     &ld->reconnect,
		     "Disable automatic reconnect-attempts by this node, but "
		     "accept incoming");
	clnopt_noarg("--dev-no-reconnect-private", OPT_DEV, opt_set_invbool,
		     &ld->reconnect_private,
		     "Disable automatic reconnect-attempts to peers with "
		     "private channel(s) only, but accept incoming");
	clnopt_noarg("--dev-fast-reconnect", OPT_DEV, opt_set_bool,
		     &ld->dev_fast_reconnect,
		     "Make max default reconnect delay 3 (not 300) seconds");

	clnopt_noarg("--dev-fail-on-subdaemon-fail", OPT_DEV, opt_set_bool,
		     &ld->dev_subdaemon_fail, opt_hidden);
	clnopt_witharg("--dev-disconnect=<filename>", OPT_DEV,
		       opt_subd_dev_disconnect, NULL, ld,
		       "File containing disconnection points");
	clnopt_noarg("--dev-allow-localhost", OPT_DEV, opt_set_bool,
		     &ld->dev_allow_localhost,
		     "Announce and allow announcments for localhost address");
	clnopt_witharg("--dev-deeponiond-poll", OPT_DEV | OPT_SHOWINT,
		       opt_set_u32, opt_show_u32, &ld->topology->poll_seconds,
		       "Time between polling for new transactions");
	clnopt_noarg("--dev-fast-gossip", OPT_DEV, opt_set_bool,
		     &ld->dev_fast_gossip,
		     "Make gossip broadcast 1 second, etc");
	clnopt_noarg("--dev-fast-gossip-prune", OPT_DEV, opt_set_bool,
		     &ld->dev_fast_gossip_prune,
		     "Make gossip pruning 30 seconds");
	clnopt_witharg("--dev-gossip-time", OPT_DEV | OPT_SHOWINT, opt_set_u32,
		       opt_show_u32, &ld->dev_gossip_time,
		       "UNIX time to override gossipd to use.");
	clnopt_witharg("--dev-force-privkey", OPT_DEV, opt_force_privkey, NULL,
		       ld, "Force HSM to use this as node private key");
	clnopt_witharg("--dev-force-bip32-seed", OPT_DEV, opt_force_bip32_seed,
		       NULL, ld, "Force HSM to use this as bip32 seed");
	...
	clnopt_witharg("--dev-max-funding-unconfirmed-blocks",
		       OPT_DEV | OPT_SHOWINT, opt_set_u32, opt_show_u32,
	...
	clnopt_noarg(
	    "--dev-no-htlc-timeout", OPT_DEV, opt_set_bool,
	    &ld->dev_no_htlc_timeout,
	    "Don't kill channeld if HTLCs not confirmed within 30 seconds");
	clnopt_noarg("--dev-fail-process-onionpacket", OPT_DEV, opt_set_bool,
		     &dev_fail_process_onionpacket,
		     "Force all processing of onion packets to fail");
	clnopt_noarg("--dev-no-version-checks", OPT_DEV, opt_set_bool,
		     &ld->dev_no_version_checks,
		     "Skip calling subdaemons with --version on startup");
	clnopt_witharg(
	    "--dev-force-features", OPT_DEV, opt_force_featureset, NULL, ld,
	    "Force the init/globalinit/node_announce/channel/bolt11/ features, "
	    "each comma-separated bitnumbers OR a single +/-<bitnumber>");
	clnopt_witharg("--dev-timeout-secs", OPT_DEV | OPT_SHOWINT, opt_set_u32,
		       opt_show_u32, &ld->config.connection_timeout_secs,
		       "Seconds to timeout if we don't receive INIT from peer");
	clnopt_noarg("--dev-no-modern-onion", OPT_DEV, opt_set_bool,
		     &ld->dev_ignore_modern_onion,
		     "Ignore modern onion messages");
	clnopt_witharg("--dev-disable-commit-after", OPT_DEV | OPT_SHOWINT,
		       opt_set_intval, opt_show_intval, &ld->dev_disable_commit,
		       "Disable commit timer after this many commits");
	clnopt_noarg("--dev-no-ping-timer", OPT_DEV, opt_set_bool,
	...
	clnopt_witharg("--dev-allowdustreserve", OPT_DEV | OPT_SHOWBOOL,
		       ...
		       "If true, we allow the `fundchannel` RPC command and "
		       "the `openchannel` plugin hook to set a reserve that is "
		       "below the dust limit.");
	clnopt_noarg("--dev-any-channel-type", OPT_DEV, opt_set_bool,
	...
	clnopt_noarg("--dev-allow-shutdown-destination-change", OPT_DEV,
		     opt_set_bool, &ld->dev_allow_shutdown_destination_change,
		     "Allow destination override on close, even if risky");
	clnopt_noarg("--dev-hsmd-no-preapprove-check", OPT_DEV, opt_set_bool,
		     &ld->dev_hsmd_no_preapprove_check,
		     "Tell hsmd not to support preapprove_check msgs");
	clnopt_noarg(
	    "--dev-hsmd-fail-preapprove", OPT_DEV, opt_set_bool,
	...
	clnopt_witharg("--dev-fd-limit-multiplier", OPT_DEV | OPT_SHOWINT,
		       opt_set_u32, opt_show_u32, &ld->fd_limit_multiplier,
		       "Try to set fd limit to this many times by number of "
		       "channels (default: 2)");
	clnopt_noarg("--dev-handshake-no-reply", OPT_DEV, opt_set_bool,
		     &ld->dev_handshake_no_reply,
		     "Don't send or read init message after connection");
	clnopt_noarg("--dev-strict-forwarding", OPT_DEV, opt_set_bool,
		     &ld->dev_strict_forwarding,
		     "Forward HTLCs along the channel specified");
	clnopt_noarg("--dev-throttle-gossip", OPT_DEV, opt_set_bool,
		     &ld->dev_throttle_gossip,
		     "Throttle gossip right down, for testing");
	/* This is handled directly in daemon_developer_mode(), so we ignore it
	 * here */
	clnopt_noarg("--dev-debug-self", OPT_DEV, opt_ignore, NULL,
		     "Fire up a terminal window with a debugger in it on "
		     "initialization");
}
...
static const struct config testnet_config = {
     ...
     */
    /* FIXME: Typo in spec for CLTV in descripton!  But it breaks our spelling
       check, so we omit it above */
    .max_htlc_cltv = 2016,
...
static const struct config mainnet_config = {
    ...
    /* While up to 483 htlcs are possible we do 30 by default (as eclair does)
       to save blockspace */
...
static void check_config(struct lightningd *ld)
{
	 ...
	 */
	if (ld->config.max_concurrent_htlcs < 1 ||
	    ld->config.max_concurrent_htlcs > 483)
		fatal("--max-concurrent-htlcs value must be between 1 and 483 "
		      "it is: %u",
...
{
	const char **features =
	    list_supported_features(tmpctx, ld->our_features);
...
static char *opt_set_msat(const char *arg, struct amount_msat *amt)
{
	if (!parse_amount_msat(amt, arg, strlen(arg)))
		return tal_fmt(tmpctx, "Unable to parse millisatoshi '%s'",
			       arg);
...
static char *opt_set_dual_fund(struct lightningd *ld)
{
	...
	feature_set_or(ld->our_features,
			   ...
			   NULL, OPTIONAL_FEATURE(OPT_STATIC_REMOTEKEY))));
	feature_set_or(ld->our_features,
		       take(feature_set_for_feature(
			   NULL, OPTIONAL_FEATURE(OPT_DUAL_FUND))));
	return NULL;
}
...
static char *opt_set_splicing(struct lightningd *ld)
{
	feature_set_or(ld->our_features,
		       take(feature_set_for_feature(
			   NULL, OPTIONAL_FEATURE(OPT_EXPERIMENTAL_SPLICE))));
	return NULL;
}
...
static char *opt_set_shutdown_wrong_funding(struct lightningd *ld)
{
	feature_set_or(
	    ld->our_features,
	    take(feature_set_for_feature(
		NULL, OPTIONAL_FEATURE(OPT_SHUTDOWN_WRONG_FUNDING))));
	return NULL;
}
...
static char *opt_set_peer_storage(struct lightningd *ld)
{
	feature_set_or(
	    ld->our_features,
	    take(feature_set_for_feature(
		NULL, OPTIONAL_FEATURE(OPT_PROVIDE_PEER_BACKUP_STORAGE))));
	feature_set_or(
	    ld->our_features,
	    take(feature_set_for_feature(
		NULL, OPTIONAL_FEATURE(OPT_WANT_PEER_BACKUP_STORAGE))));
	return NULL;
}

static char *opt_set_quiesce(struct lightningd *ld)
{
	if (!opt_deprecated_ok(ld, "experimental-quiesce", NULL, "v24.11",
			       "v25.05"))
...
{
	if (!opt_deprecated_ok(ld, "experimental-anchors", NULL, "v24.02",
			       "v25.02"))
...
{
	if (!opt_deprecated_ok(ld, "announce-addr-dns", NULL, "v23.08",
			       "v24.08"))
		return "--announce-addr-dns has been deprecated, use "
		       "--bind-addr=dns:...";
	return opt_set_bool_arg(optarg, &ld->announce_dns);
}

char *hsm_secret_arg(const tal_t *ctx, const char *arg, const u8 **hsm_secret)
{
	char *codex32_fail;
...
static char *opt_set_codex32_or_hex(const char *arg, struct lightningd *ld)
{
	...
	if (!write_all(fd, payload, tal_count(payload))) {
		...
		return tal_fmt(tmpctx, "Writing HSM: %s", strerror(errno));
	}
...
static void register_opts(struct lightningd *ld)
{
	...
	clnopt_noarg("--test-daemons-only", OPT_EARLY | OPT_EXITS,
		     test_subdaemons_and_exit, ld,
		     "Test that subdaemons can be run, then exit immediately");
	/* We need to know this even before we talk to plugins */
	clnopt_witharg(
	    "--allow-deprecated-apis", OPT_EARLY | OPT_SHOWBOOL,
	    opt_set_bool_arg, opt_show_bool, &ld->deprecated_ok,
	...
	opt_register_early_arg(
	    "--recover", opt_set_codex32_or_hex, NULL, ld,
	    "Populate hsm_secret with the given codex32 secret"
	    " and starts the node in `offline` mode.");

	/* This affects our features, so set early. */
	opt_register_early_noarg("--large-channels|--wumbo", opt_set_wumbo, ld,
				 opt_hidden);

	opt_register_early_noarg("--experimental-dual-fund", opt_set_dual_fund,
				 ld,
				 "experimental: Advertise dual-funding"
				 " and allow peers to establish channels"
				 " via v2 channel open protocol.");

	opt_register_early_noarg("--experimental-splicing", opt_set_splicing,
				 ld,
	...
	opt_register_early_noarg("--experimental-onion-messages",
				 opt_set_onion_messages, ld, opt_hidden);
	opt_register_early_noarg(
	    "--experimental-offers", opt_set_offers, ld,
	...
	opt_register_early_noarg(
	    "--experimental-peer-storage", opt_set_peer_storage, ld,
	    "EXPERIMENTAL: enable peer backup storage and restore");
	opt_register_early_noarg("--experimental-quiesce", opt_set_quiesce, ld,
	...
	clnopt_witharg("--announce-addr-dns", OPT_EARLY | OPT_SHOWBOOL,
		       opt_set_bool_arg, opt_show_bool, &ld->announce_dns,
		       opt_hidden);

	clnopt_noarg("--help|-h", OPT_EXITS, opt_lightningd_usage, ld,
		     "Print this message.");
	...
	opt_register_arg("--pid-file=<file>", opt_set_talstr, opt_show_charp,
			 &ld->pidfile, "Specify pid file");

	clnopt_witharg("--ignore-fee-limits", OPT_SHOWBOOL, opt_set_bool_arg,
		       opt_show_bool, &ld->config.ignore_fee_limits,
		       "(DANGEROUS) allow peer to set any feerate");
	clnopt_witharg("--watchtime-blocks", OPT_SHOWINT, opt_set_u32,
		       opt_show_u32, &ld->config.locktime_blocks,
		       "Blocks before peer can unilaterally spend funds");
	opt_register_arg("--max-locktime-blocks", opt_set_max_htlc_cltv, NULL,
			 ld, opt_hidden);
	clnopt_witharg("--funding-confirms", OPT_SHOWINT, opt_set_u32,
		       opt_show_u32, &ld->config.anchor_confirms,
	...
	clnopt_witharg("--require-confirmed-inputs", OPT_SHOWBOOL,
		       ...
		       "Confirmations required for inputs to funding "
		       "transaction (v2 opens only)");
	...
	clnopt_witharg("--commit-time=<millseconds>", OPT_SHOWINT, opt_set_u32,
		       opt_show_u32, &ld->config.commit_time_ms,
	...
	clnopt_witharg("--fee-per-satoshi", OPT_SHOWINT, opt_set_u32,
		       opt_show_u32, &ld->config.fee_per_satoshi,
		       "Microsatoshi fee for every satoshi in HTLC");
	clnopt_witharg("--htlc-minimum-msat", OPT_SHOWMSATS, opt_set_msat,
		       opt_show_msat, &ld->config.htlc_minimum_msat,
		       "The default minimal value an HTLC must carry in order "
		       "to be forwardable for new channels");
	clnopt_witharg("--htlc-maximum-msat", OPT_SHOWMSATS, opt_set_msat,
		       opt_show_msat, &ld->config.htlc_maximum_msat,
		       "The default maximal value an HTLC must carry in order "
		       "to be forwardable for new channel");
	clnopt_witharg("--max-concurrent-htlcs", OPT_SHOWINT, opt_set_u32,
		       opt_show_u32, &ld->config.max_concurrent_htlcs,
		       "Number of HTLCs one channel can handle concurrently. "
		       "Should be between 1 and 483");
	...
	clnopt_witharg("--addr", OPT_MULTI, opt_add_addr, NULL, ld,
		       "Set an IP address (v4 or v6) to listen on and announce "
		       "to the network for incoming connections");
	clnopt_witharg(
	    "--bind-addr", OPT_MULTI, opt_add_bind_addr, NULL, ld,
	    "Set an IP address (v4 or v6) to listen on, but not announce");
	clnopt_witharg("--announce-addr", OPT_MULTI, opt_add_announce_addr,
		       NULL, ld,
		       "Set an IP address (v4 or v6) or .onion v3 to announce, "
		       "but not listen on");

	opt_register_arg("--announce-addr-discovered", opt_set_autobool_arg,
			 opt_show_autobool, &ld->config.ip_discovery,
			 "Explicitly turns IP discovery 'on' or 'off'.");
	clnopt_witharg(
	    "--announce-addr-discovered-port", OPT_SHOWINT, opt_set_uintval,
	    opt_show_uintval, &ld->config.ip_discovery_port,
	    "Sets the public TCP port to use for announcing discovered IPs.");
	opt_register_noarg("--offline", opt_set_offline, ld,
			   "Start in offline-mode (do not automatically "
			   "reconnect and do not accept incoming connections)");
	clnopt_witharg("--autolisten", OPT_SHOWBOOL, opt_set_bool_arg,
		       opt_show_bool, &ld->autolisten,
		       "If true, listen on default port and announce if it "
		       "seems to be a public interface");
	opt_register_arg("--proxy", opt_add_proxy_addr, NULL, ld,
			 "Set a socks v5 proxy IP address and port");
	...
	opt_register_arg("--accept-htlc-tlv-types",
			 opt_set_accept_extra_tlv_types, NULL, ld, opt_hidden);
	...
	opt_register_early_noarg("--disable-dns", opt_set_invbool,
				 &ld->config.use_dns,
				 "Disable DNS lookups of peers");

	opt_register_noarg("--encrypted-hsm", opt_set_hsm_password, ld,
			   "Set the password to encrypt hsm_secret with. If no "
			   "password is passed through command line, "
	...
	opt_register_arg("--force-feerates", opt_force_feerates, NULL, ld,
			 "Set testnet/regtest feerates in sats perkw, "
			 "opening/mutual_close/unlateral_close/delayed_to_us/"
			 "htlc_resolution/penalty: if fewer specified, last "
			 "number applies to remainder");

	clnopt_witharg("--commit-fee", OPT_SHOWINT, opt_set_u64, opt_show_u64,
		       &ld->config.commit_fee_percent,
		       "Percentage of fee to request for their commitment");
	clnopt_witharg("--commit-feerate-offset", OPT_SHOWINT, opt_set_u32,
		       opt_show_u32, &ld->config.feerate_offset,
	...
	clnopt_witharg("--subdaemon", OPT_MULTI, opt_subdaemon, NULL, ld,
		       "Arg specified as SUBDAEMON:PATH. "
	...
	opt_register_noarg(
	    "--experimental-upgrade-protocol", opt_set_bool,
	    &ld->experimental_upgrade_protocol,
	    "experimental: allow channel types to be upgraded on reconnect");
	opt_register_noarg("--invoices-onchain-fallback", opt_set_bool,
			   &ld->unified_invoices,
			   "Include an onchain address in invoices and mark "
			   "them as paid if payment is received on-chain");
	clnopt_witharg(
	    "--database-upgrade", OPT_SHOWBOOL, opt_set_db_upgrade, NULL, ld,
	    "Set to true to allow database upgrades even on non-final releases "
	    "(WARNING: you won't be able to downgrade!)");
	clnopt_witharg("--i-promise-to-fix-broken-api-user", OPT_MULTI,
		       opt_add_api_beg, NULL, ld,
		       "Re-enable a long-deprecated API (which will be removed "
		       "entirely next version!)");
...
static void promote_missing_files(struct lightningd *ld)
{
	...
	while ((d = readdir(d_from)) != NULL) {
		...
		if (streq(d->d_name, ".") || streq(d->d_name, "..") ||
		    streq(d->d_name, "config") || strends(d->d_name, ".pid"))
			continue;
		...
		if (lstat(d->d_name, &st) != -1)
			errx(1, "Refusing to overwrite %s into %s/", fullname,
			     ld->config_netdir);
		log_unusual(ld->log, "Moving %s into %s/", d->d_name,
			    ld->config_netdir);
		if (rename(fullname, d->d_name) != 0)
			err(1, "Could not move %s/%s to %s", ld->config_basedir,
...
/* Names stolen from
 * https://github.com/ternus/nsaproductgenerator/blob/master/nsa.js */
static const char *codename_adjective[] = {
    "LOUD",    "RED",	 "BLUE",   "GREEN",   "YELLOW",	  "IRATE",
    "ANGRY",   "PEEVED", "HAPPY",  "SLIMY",   "SLEEPY",	  "JUNIOR",
    "SLICKER", "UNITED", "SOMBER", "BIZARRE", "ODD",	  "WEIRD",
    "WRONG",   "LATENT", "CHILLY", "STRANGE", "LOUD",	  "SILENT",
    "HOPPING", "ORANGE", "VIOLET", "VIOLENT", "LIGHTNING"};

static const char *codename_noun[] = {
    "WHISPER",	 "FELONY",  "MOON",    "SUCKER",  "PENGUIN",  "WAFFLE",
    "MAESTRO",	 "NIGHT",   "TRINITY", "DEITY",	  "MONKEY",   "ARK",
    "SQUIRREL",	 "IRON",    "BOUNCE",  "FARM",	  "CHEF",     "TROUGH",
    "NET",	 "TRAWL",   "GLEE",    "WATER",	  "SPORK",    "PLOW",
    "FEED",	 "SOUFFLE", "ROUTE",   "BAGEL",	  "MONTANA",  "ANALYST",
    "AUTO",	 "WATCH",   "PHOTO",   "YARD",	  "SOURCE",   "MONKEY",
    "SEAGULL",	 "TOLL",    "SPAWN",   "GOPHER",  "CHIPMUNK", "SET",
    "CALENDAR",	 "ARTIST",  "CHASER",  "SCAN",	  "TOTE",     "BEAM",
    "ENTOURAGE", "GENESIS", "WALK",    "SPATULA", "RAGE",     "FIRE",
    "MASTER"};
...
void setup_color_and_alias(struct lightningd *ld)
{
	...
	if (!ld->alias) {
		...
		memcpy(&noun, ld->our_nodeid.k + 3 + sizeof(adjective),
		       sizeof(noun));
		noun %= ARRAY_SIZE(codename_noun);
		adjective %= ARRAY_SIZE(codename_adjective);

		/* Only use 32 characters */
		name = tal_fmt(ld, "%s%s", codename_adjective[adjective],
			       codename_noun[noun]);
...
void handle_early_opts(struct lightningd *ld, int argc, char *argv[])
{
	...
	clnopt_noarg("--list-features-only", OPT_EARLY | OPT_EXITS,
		     list_features_and_exit, ld,
		     "List the features configured, and exit immediately");

	/*~ We need to know this super-early, as it controls other options */
	clnopt_noarg("--developer", OPT_EARLY | OPT_SHOWBOOL, opt_set_bool,
		     &ld->developer,
		     "Enable developer commands/options, disable legacy APIs");

	/*~ This does enough parsing to get us the base configuration options */
	ld->configvars = initial_config_opts(
	    ld, &argc, argv, true, &ld->config_filename, &ld->config_basedir,
	    &ld->config_netdir, &ld->rpc_filename);
	...
	ld->wallet_dsn =
	    tal_fmt(ld, "sqlite3://%s/lightningd.sqlite3", ld->config_netdir);

	/* Set default PID file name to be per-network (in base dir) */
	ld->pidfile = path_join(
	    ld, ld->config_basedir,
	    tal_fmt(tmpctx, "lightningd-%s.pid", chainparams->network_name));
	...
	if (chdir(ld->config_netdir) != 0) {
		...
		if (mkdir(ld->config_basedir, 0700) != 0 && errno != EEXIST)
			fatal("Could not make directory %s: %s",
			      ld->config_basedir, strerror(errno));
...
static void fixup_clnrest_options(struct lightningd *ld)
{
	for (size_t i = 0; i < tal_count(ld->configvars); i++) {
		...
		if (opt_find_long(cv->configline,
				  cast_const2(const char **, &cv->optarg)))
			continue;
		if (!opt_deprecated_ok(
			ld,
			tal_strndup(tmpctx, cv->configline,
				    strcspn(cv->configline, "=")),
			"clnrest-prefix", "v23.11", "v24.11"))
			continue;
		log_unusual(ld->log,
			    "Option %s deprecated in v23.11, renaming to cln%s",
...
void handle_opts(struct lightningd *ld)
{
	...
	if (tal_count(ld->proposed_wireaddr) != 0 &&
		...
		if (!ld->proxyaddr)
			log_info(ld->log,
				 "Pure Tor setup with no --proxy:"
				 " you won't be able to make connections out");
	}
	check_config(ld);
}

static void json_add_opt_addrs(struct json_stream *response, const char *name0,
...
static void json_add_opt_log_to_files(struct json_stream *response,
				      const char *name0, const char **logfiles)
{
...
static bool
json_add_opt_alt_subdaemon(const char *member, const char *value,
			   struct json_add_opt_alt_subdaemon_args *argp)
{
	json_add_string(argp->response, argp->name0,
...
void add_config_deprecated(struct lightningd *ld, struct json_stream *response,
			   const struct opt_table *opt, const char *name,
			   size_t len)
{
	...
	if (opt->type & OPT_NOARG) {
		if (opt->cb == (void *)opt_clear_plugins) {
			...
			json_add_bool(response, "encrypted-hsm",
				      ld->encrypted_hsm);
		} else if (opt->cb == (void *)opt_set_wumbo) {
			json_add_bool(response, name0,
				      feature_offered(
					  ld->our_features->bits[INIT_FEATURE],
					  OPT_LARGE_CHANNELS));
		...
		/* We ignore future additions, since these are deprecated
		 * anyway! */
		...
		if (opt->show == (void *)opt_show_charp) {
			if (*(char **)opt->u.carg)
				...
				answer =
				    tal_strdup(tmpctx, *(char **)opt->u.carg);
		} else if (opt->show) {
			strcpy(buf + sizeof(buf) - sizeof("..."), "...");
			if (!opt->show(buf, sizeof(buf) - sizeof("..."),
				       opt->u.carg))
				buf[0] = '\0';
			...
			if (opt->type & OPT_CONCEAL) {
				...
				if (streq(buf, "") ||
					...
					errx(1, "Bad literal for %s: %s", name0,
					     buf);
			...
			if (ld->logfiles)
				json_add_opt_log_to_files(response, name0,
							  ld->logfiles);
			...
			json_add_opt_addrs(
			    response, name0, ld->proposed_wireaddr,
			    ld->proposed_listen_announce, ADDR_LISTEN);
			return;
		} else if (opt->cb_arg == (void *)opt_add_announce_addr) {
			json_add_opt_addrs(
			    response, name0, ld->proposed_wireaddr,
			    ld->proposed_listen_announce, ADDR_ANNOUNCE);
			return;
			...
			/* We ignore this: it's printed below: */
			   ...
			   (void *)opt_set_accept_extra_tlv_types) {
			for (size_t i = 0;
			     i < tal_count(ld->accept_extra_tlv_types); i++) {
				if (i == 0)
					answer = tal_fmt(
					    name0, "%" PRIu64,
